package shared;

public class Constants {

    public static final int[] SBOX = {
            
    };

    public static final int[] INV_SBOX = {  };

    // AES Rcon (Round constants)
    public static final int[] RCON = {
            0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36
    };

    public static final int blockSizeInWords = 4;  // Block size in words (4 words = 16 bytes = 128 bits)
    public static final int blockSizeInBytes = 16; // AES block size is 16 bytes (128 bits)
    public static final int keyLengthInWords = 4;  // Key length in words (AES-128 uses 4 words = 128 bits)
    public static final int numberOfRounds = 10;   // Number of rounds of encryption (10 for AES-128)

}
